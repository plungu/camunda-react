...
COPY --from=builder /usr/src/app/target/camunda-reactjs.jar ${WORKDIR}
# run the application
ENTRYPOINT ["java","-Dspring.profiles.active=cors,prod","-Djava.security.egd=file:/dev/./urandom","-jar","/usr/src/app/camunda-reactjs.jar"]
